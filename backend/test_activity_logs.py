...
def test_user_dob_and_gender():
    """Test that DOB and Gender can be set on registration and retrieved."""
    with TestClient(app) as client:
        # 1. Register a user with DOB and Gender
        user_data = {
            "email": "dob_gender_student@edumanage.com",
            "password": "password123",
            "full_name": "DOB Gender Student",
            "phone": "+919876543211",
            "role": "student",
            "dob": "1995-05-15",
            "gender": "Female"
        ...
        # 3. Call the /me endpoint to get user info
        me_response = client.get("/api/auth/me", headers={"Authorization": f"Bearer {token}"})
        assert me_response.status_code == 200

        user_info = me_response.json()
        assert user_info["dob"].startswith("1995-05-15")
